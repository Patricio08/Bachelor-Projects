...
static inline uthread_t * extract_next_thread() {
	...
	list_entry_t * next_pentry = ready_queue.next;
	uthread_t * uthread = container_of(next_pentry, uthread_t, entry);
	
	for(list_entry_t * pentry = ready_queue.next; pentry != &ready_queue; pentry = pentry->next) {
		uthread_t * ut;
		ut = container_of(pentry, uthread_t, entry);

		if (ut->priority > uthread->priority) {
			next_pentry = pentry;
			uthread = container_of(next_pentry, uthread_t, entry);
		}

	}
	remove_list_entry(next_pentry);
...
//
// Returns handle thread status
//
enum STATUS ut_status(uthread_t * handle){
...
//
uthread_t* ut_create(void (*start_routine)(void *), void * arg, int priority) {
	uthread_t * thread;
